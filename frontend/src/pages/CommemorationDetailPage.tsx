...
export function CommemorationDetailPage() {
  ...
  const erreurMetier = (err: unknown, fallback: string) =>
    toast.error(fallback, err instanceof ApiError ? err.message : t('commemorations.detail.toast.reessayez'))
  ...
  const changerStatut = async (statut: StatutCommemoration) => {
    ...
    try {
      ...
      setItem(maj)
      toast.success(t('commemorations.detail.toast.statutMaj'), t(`commemorations.statut.${statut}`))
    } catch (err) {
      erreurMetier(err, t('commemorations.detail.toast.statutImpossible'))
    } finally {
  ...
  const supprimer = async () => {
    ...
    try {
      ...
      toast.success(t('commemorations.detail.toast.supprimee'), item.titre)
      navigate('/commemorations')
    } catch (err) {
      erreurMetier(err, t('commemorations.detail.toast.suppressionImpossible'))
      setDeleting(false)
  ...
  return (
    <>
        ...
        <dl className="mt-5 grid gap-4 border-t border-hairline pt-5 text-sm sm:grid-cols-2">
          ...
          {item.lieu && (
            <div className="flex items-center gap-2">
              ...
              <dt className="text-muted-foreground">{t('commemorations.detail.lieu')}</dt>
        ...
        {item.membresConcernes.length > 0 && (
          <div className="mt-5 border-t border-hairline pt-5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">{t('commemorations.detail.membresHonores')}</span>
            </div>
      ...
      {gestion && (
        <Card className="nk-reveal nk-d3 mt-6 p-6">
          <div className="flex items-center gap-2">
            ...
            <Overline>{t('commemorations.detail.statut')}</Overline>
          </div>
          <div className="mt-4 max-w-xs">
            <Field label={t('commemorations.detail.statutEvenement')}>
              <Select
                value={item.statut}
      ...
      {item.notes?.trim() && (
        <Card className="nk-reveal nk-d3 mt-6 p-6">
          <Overline>{t('commemorations.detail.notes')}</Overline>
