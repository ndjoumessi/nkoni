...
| 0.4 | **Canal de notification fiable** — finaliser les **templates WhatsApp pré-approuvés Meta** (compte Meta Business) **ou** brancher un repli (email transactionnel / SMS). → **Repli EMAIL LIVRÉ** (`ce0ce25`) : `services/email.service.ts` via **Resend** (pièce jointe base64), best-effort (n'échoue jamais l'op métier), no-op sans config, + champ `Membre.email`. `env.ts` avertit en prod si AUCUN canal (ni WhatsApp `WHATSAPP_TOKEN`/`WHATSAPP_PHONE_ID`, ni email `RESEND_API_KEY`/`RESEND_FROM`) n'est configuré. **Reste à faire (action PO)** : poser `RESEND_API_KEY` + `RESEND_FROM` sur Railway (canal minimal) ; les templates WhatsApp Meta restent optionnels tant que le compte Meta Business n'existe pas. | Les relances de cotisation et l'envoi de reçus sont un cœur de valeur ; le code existe (WhatsApp + repli email) mais reste un no-op tant qu'aucun canal n'est configuré en prod. Sans canal, la promesse produit n'est pas tenue. | Moyen |
