...
/**
 * Pourcentage localisé jusqu'à 2 décimales, SANS le signe `%` (ajouté par l'appelant) — miroir
 * de `formatPourcent` du front. Utilisé pour les VARIATIONS des rapports : contrairement à
 * `formatNombre` (0 décimale, pour les montants entiers), on conserve les décimales (ex. -2,78).
 */
export function formatPourcentage(n: number, langue: Langue): string {
  return new Intl.NumberFormat(LOCALE_PAR_LANGUE[langue] ?? 'fr', {
