...
| 2.2 | **Support & incidents** — page de statut publique, canal de support (email/formulaire), runbook d'incident et de communication, définition d'un SLA de disponibilité. → **Part LIVRÉE** : page de statut publique `/statut` (interroge `/health`, état opérationnel/incident/vérification, i18n FR/EN) + contact support (mailto), liée au footer. **Reste à faire** : runbook d'incident + communication, définition d'un SLA, éventuel formulaire de support (réutilisable via le client Resend) et historique d'incidents. | Cadre d'exploitation attendu d'un SaaS payant. Le `RUNBOOK_bascule_prod` existe déjà comme base. | Faible/Moyen |
