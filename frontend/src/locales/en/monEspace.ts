/** EN strings — member self-service space (§5). Strict parity with FR. */
export default {
  monEspace: {
    titre: 'My space',
    sousTitre: 'Your situation, your contributions and your receipts.',
    tabs: {
      aria: 'My space sections',
      apercu: 'Overview',
      contributions: 'Contributions',
      tontines: 'Tontines',
      recus: 'Receipts',
    },
    aucuneFiche: {
      titre: 'No member profile',
      texte: 'No member profile is linked to your account.',
    },
    situation: {
      titre: 'My situation',
      totalDu: 'Total due',
      totalVerse: 'Total paid',
      reste: 'Left to pay',
      branche: 'Branch',
      anneeAdhesion: 'Member since',
      statut: 'Status',
      progression: 'Progress',
      aJourMerci: 'You are up to date with your contributions. Thank you!',
    },
    carte: {
      titre: 'My member card',
      membreLabel: 'MEMBER CARD',
      branche: 'Branch',
      depuis: 'Member since',
      scanner: 'Scan to check status',
      qrAlt: 'QR code to verify this member\'s status',
      chef: 'Head',
      telecharger: 'Download the PDF',
      erreur: 'Could not download the card',
    },
    rappels: {
      titre: 'My notifications',
      nonLu: 'Unread',
      nonLus_one: '{{count}} unread',
      nonLus_other: '{{count}} unread',
      autres_one: '+{{count}} more notification',
      autres_other: '+{{count}} more notifications',
      toutLu: 'Mark all as read',
      marquerLu: 'Mark as read',
      supprimer: 'Dismiss notification',
    },
    paiement: {
      payer: 'Pay',
      payerReste: 'Pay the balance ({{montant}})',
      succes: 'Payment confirmed. Thank you!',
      echec: 'Could not start the payment',
      echecPaiement: 'The payment did not go through',
      enAttente: 'Payment awaiting confirmation…',
      validezTelephone: 'Approve the payment on your phone (Mobile Money prompt), then wait a moment.',
      titre: 'Pay my {{annee}} contribution',
      resteDu: 'Remaining due',
      montantLabel: 'Amount to pay',
      montantAide: 'Pay all or part of the remaining balance.',
      montantInvalide: 'Invalid amount: at least the minimum, no more than the remaining balance.',
      confirmer: 'Pay',
      annuler: 'Cancel',
    },
    contributions: {
      titre: 'My contributions',
      annee: 'Year',
      attendu: 'Expected',
      verse: 'Paid',
      valorise: 'Valued',
      statut: 'Status',
      aucune: 'No contribution recorded.',
      detailVersements: 'Payments',
    },
    reunions: {
      titre: 'Upcoming meetings',
      lieu: 'Location',
      aucune: 'No upcoming meeting.',
      rsvpQuestion: 'Will you attend?',
      rsvpEnregistre: 'Response saved.',
      rsvp: {
        PRESENT: 'Attending',
        EXCUSE: 'Excused',
        ABSENT: 'Absent',
      },
    },
    votes: {
      titre: 'Open votes',
      reunion: 'Meeting',
      question: 'Your vote:',
      enregistre: 'Vote saved.',
      sens: {
        POUR: 'For',
        CONTRE: 'Against',
        ABSTENTION: 'Abstain',
      },
    },
    recus: {
      titre: 'My receipts',
      numero: 'Number',
      date: 'Date',
      montant: 'Amount',
      voir: 'View',
      apercuTitre: 'Receipt {{numero}}',
      ouvrirPdf: 'Open the PDF',
      membre: 'Member',
      dateVersement: 'Payment date',
      mode: 'Method',
      statut: 'Status',
      telecharger: 'Download',
      indisponible: 'PDF unavailable',
      annule: 'Cancelled',
      aucun: 'No receipt available.',
      aucunActif: 'No downloadable receipt yet.',
      annulesGroupe_one: 'Cancelled receipt ({{count}})',
      annulesGroupe_other: 'Cancelled receipts ({{count}})',
    },
    tontines: {
      titre: 'My tontines',
      aucune: 'You are not part of any tontine.',
      cycle: 'Cycle {{numero}}',
      monRang: 'My order',
      maMise: 'My stake per round',
      tour: 'Round {{numero}}',
      beneficiaire: 'I receive the pot',
      misePayee: 'Stake paid',
      misePartielle: 'Partial: {{montant}}',
      miseDue: 'Stake due',
      pot: 'Pot',
    },
    amendes: {
      titre: 'My fines',
      aucune: 'No fines against you.',
      motif: 'Reason',
      montant: 'Amount',
      date: 'Date',
      statut: 'Status',
      payeeLe: 'Paid on {{date}}',
    },
    cagnottes: {
      titre: 'My collective funds',
      aucune: 'No open fund.',
      objectif: 'Goal',
      collecte: 'Collected',
      monDon: 'My donation',
      aucunDon: 'No donation from me',
    },
  },
}
