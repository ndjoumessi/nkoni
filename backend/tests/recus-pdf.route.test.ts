...
function buildMock(membreCompteId: string | null) {
  const prisma: any = {
    recu: {
      findUnique: async ({ where }: any) =>
        where.id === 'r1'
          ? { id: 'r1', numero: 'NKONI-2024-000001', dateGeneration: new Date('2024-03-01'), versementId: 'v1', urlPdf: null }
          : null,
      update: async () => ({}),
    },
    versement: {
...
describe('Reçu PDF + WhatsApp — accès', () => {
  ...
  async function demarrer(membreCompteId: string | null) {
    const m = buildMock(membreCompteId)
    whatsapp = m.whatsapp
