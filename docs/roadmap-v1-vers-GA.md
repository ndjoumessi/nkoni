...
| 2.2 | **Support & incidents** — page de statut publique, canal de support (email/formulaire), runbook d'incident et de communication, définition d'un SLA de disponibilité. → **LIVRÉ** : page de statut publique `/statut` (interroge `/health`, état opérationnel/incident/vérification, i18n FR/EN) + contact support (mailto), liée au footer · **`docs/RUNBOOK_incidents.md`** (gravités P1/P2/P3, boucle contenir-avant-diagnostiquer, 7 leviers de confinement du rollback à la restauration, catalogue de pannes, modèles de communication FR/EN, régime RGPD des fuites, post-mortem, journal) · **`docs/SLA_disponibilite.md`** (99 % mensuel, délais de réaction par gravité, maintenance, exclusions). **⚠️ Le SLA est une cible SANS INSTRUMENT** tant que les 3 prérequis du runbook §8 ne sont pas faits : (8.1) poser `SENTRY_DSN`/`VITE_SENTRY_DSN` — **absents de la prod, la couche d'alerte est dormante** ; (8.2) sonde externe sur `/api/health` — sans elle la disponibilité n'est pas mesurable ; (8.3) faire de `/health` un vrai healthcheck — il ne touche pas la base, donc `/statut` peut publier un « opérationnel » faux. **Reste optionnel** : bandeau d'incident éditable sur `/statut` (aujourd'hui purement automatique), historique d'incidents, formulaire de support (réutilisable via le client Resend). | Cadre d'exploitation attendu d'un SaaS payant. Le `RUNBOOK_bascule_prod` existe déjà comme base. | Faible/Moyen |
